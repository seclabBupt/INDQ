...
#include <zlib.h> // for boost::crc_32_type
...
static char redundancy = 2;
static struct keywriteEntry *storage;
	...
	int buffer_size)
{
	...
	size = buffer_size;
	num_entries = size / sizeof(struct keywriteEntry);
	int i;
	int mr_flags = 0;
	int cq_size = 0;
	int num_devices;
	...
	storage = (struct keywriteEntry *)res->buf;
...
int storeMetadata(struct resources res)
{
	...
	json_object_object_add(json_obj, "psn", json_object_new_int(getRQPSN(res)));
...
// 	uLong crc32_value = crc32(0L, Z_NULL, 0); 
// 	uLong checksum = crc32_value;
...
// 	checksum = crc32(checksum, buffer_key, sizeof(buffer_key));
...
// 		uLong checksum_copy = crc32_value;
// 		slot = crc32(checksum_copy, buffer, sizeof(buffer));
...
uint32_t ip_str_to_int(const char *ip) {
    ...
    return htonl(result); // 转换为网络序
}
// 
//uint32_t queryFiveTuples(const char *src_ip, uint16_t src_port, const char *dst_ip, uint16_t dst_port, uint8_t protocol,)
// {
// 	char buffer_key[13]; // This is the crc input
// 	char buffer[14];

// 	uint32_t crc = crc32(0L, Z_NULL, 0); // 初始化CRC32
// 	uLong checksum = crc;
//     // 转换IP地址为整数并添加到CRC计算中
//     uint32_t src_ip_int = ip_str_to_int(src_ip);
// 	uint32_t dst_ip_int = ip_str_to_int(dst_ip);
// 	uint32_t slot; // this will be store calculated keyval slots

// 	memcpy(buffer_key, &src_ip_int, 4);       // 源IP地址
//     memcpy(buffer_key + 4, &src_port, 2);     // 源端口，使用htons转换为网络字节序
//     memcpy(buffer_key + 6, &dst_ip_int, 4);   // 目的IP地址
//     memcpy(buffer_key + 10, &dst_port, 2);   // 目的端口，使用htons转换为网络字节序
//     memcpy(buffer_key + 12, &protocol, 1);   // 协议号
//     memcpy(buffer, &buffer_key, 13);
// 	// Calculate concatenated checksum
// 	checksum = crc32(checksum, buffer_key, sizeof(buffer_key));
// 	// cout << "We are expecting checksum " << checksum << endl;
...
// 	for (char n = 0; n < redundancy; n++) // Loop through all redundancies
// 	{
// 		buffer[13] = n; // The redundancy slot will differ
// 		// Calculate the index slot for this redundancy entry
// 		uLong checksum_copy = crc;
// 		slot = crc32(checksum_copy, buffer, sizeof(buffer));
// 		slot %= num_entries; // num_entries= size / sizeof(struct keywriteEntry);
// 		// cout << "Key " << key << " redundancy n=" << (int)n << " hashed to slot " << slot << endl;
// 		// cout << "Data in this slot: " << storage[slot].data << "csum: " << storage[slot].checksum << endl;
...
int main(int argc, char *argv[])
{
	...
	printf("Press q or Q to disconnect...\n");
	while (1)
	{
		int c = getchar();
		// 如果输入字符为 'q' 或 'Q', 则直接退出循环
		if (c == 'q' || c == 'Q')
		{
			printf("Disconnecting...\n");
			break;
		}
		// 否则输出当前存储的 uint64_t 数字
		else
		{
			printStorage(res);
		}
